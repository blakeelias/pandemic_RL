...
class PandemicEnvironment(gym.Env):
  ...
  def __init__(self,
    ...
    self.lookup = dict([(x, i) for i in range(num_states - 1) for x in range(states[i], states[i+1])] + [(num_population, len(states) - 1)])
  ...
  def _reward(num_infected, r, power=power, **kwargs):
  ...
  def _expected_new_state(self, current_state, action, imported_cases_per_step=imported_cases_per_step, **kwargs):
    r_effective = action
  ...
  def _new_state_distribution(self, state, action, distr_family='poisson', **kwargs):
    ...
    lam = self.expected_new_state(state, action, **kwargs)
