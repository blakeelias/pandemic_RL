...
class PandemicCompressedEnv(gym.Env):
  ...
  def __init__(self,
    ...
    self.nS = len(self.states_num_infected)
    self.lookup = dict([(x, i) for i in range(self.nS - 1) for x in range(self.states_num_infected[i], self.states_num_infected[i+1])] + [(num_population, self.nS - 1)])
